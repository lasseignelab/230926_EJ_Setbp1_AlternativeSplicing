...
# cell metadata
file <- "phenoData.txt"
df.gene.norm.pheno <- read.table(paste(path, file, sep = ""),
  sep = "\t",
...
df.gene.norm.feature <- read.table(paste(path, file, sep = ""),
  sep = "\t",
  header = TRUE, stringsAsFactors = FALSE
)
...
# cell metadata
file <- "phenoData.txt"
df.gene.count.pheno <- read.table(paste(path, file, sep = ""),
  sep = "\t",
  header = TRUE, stringsAsFactors = FALSE
)
...
# feature metadata
file <- "featureData.txt"
df.gene.count.feature <- read.table(paste(path, file, sep = ""),
  sep = "\t",
  header = TRUE, stringsAsFactors = FALSE
)
...
# cell metadata
file <- "phenoData.txt"
df.sj.count.pheno <- read.table(paste(path, file, sep = ""),
  sep = "\t",
  header = TRUE, stringsAsFactors = FALSE
)
...
file <- "featureData.txt"
df.sj.count.feature <- read.table(paste(path, file, sep = ""),
  sep = "\t",
  header = TRUE, stringsAsFactors = FALSE
)
...
gtf <- as.data.frame(fread(paste(path, file, sep = ""),
  sep = "\t",
...
If needed, this is a stopping point where you can load your data.

```{r load marvel object if needed}
# Load object
path <- "/data/user/efjones/230926_EJ_Setbp1_AlternativeSplicing/data/MARVEL_tutorial/Data/R Object/"
file <- "MARVEL_EJ.RData"
...
Now, you can do this differential splicing analysis - this step takes about 20 min
...
This last step takes a while, so you might want to save after this step as well.

```{r save marvel object}
# Save object
path <- "/data/user/efjones/230926_EJ_Setbp1_AlternativeSplicing/data/MARVEL_tutorial/Data/R Object/"
file <- "MARVEL_EJ_splicing.RData"
save(marvel, file = paste(path, file, sep = ""))
```

If needed, this is another stopping point where you can load your data.
...
Differential gene analysis doesn't take quite as long, so no need to save after this step.
...
head(marvel$DE$SJ$VolcanoPlot$SJ$Data[, c(
  "coord.intron",
  "gene_short_name",
  "sig"
)])
...
head(marvel$SJ.Gene.Cor$Data[, c(
  "coord.intron",
  "gene_short_name",
  "cor.complete"
)])
...
# Plot cell groups
marvel <- PlotValues.PCA.CellGroup.10x(
  MarvelObject = marvel,
...
marvel <- PlotValues.PCA.Gene.10x(
  MarvelObject = marvel,
  gene_short_name = "VIM",
...
After all these visualizations, you can do downstream things like GO analysis

```{r go analysis}
marvel <- BioPathways.10x(
  MarvelObject = marvel,
  pval = 0.05,
  delta = 5,
  min.gene.norm = 1.0,
...
Getting "Error: database or disk is full", but I definitely have enough memory
...
```
```{r gene splicing relationship}
# Perform DGE analysis for all pairwise group comparison
marvel <- adhocGene.DE.Gene.10x(MarvelObject=marvel)

# Perform PSI expression analysis for all pairwise group comparison
