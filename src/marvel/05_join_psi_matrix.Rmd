...
psi_matrix@Dimnames[[2]] <- sample_metadata$cell.id

# save the final psi matrix
write_rds(psi_matrix, here::here("data", "marvel", "psi_matrix.Rds"))
```
...
Now split matrices by cell type

```{r split psi matrices by cell type}
astrocytes_psi <- subset_sparse_matrix(
  matrix = psi_matrix,
  cell_type = "Astrocytes"
)

excitatory_psi <- subset_sparse_matrix(
  matrix = psi_matrix,
  cell_type = "Excitatory Neurons"
)

inhibitory_psi <- subset_sparse_matrix(
  matrix = psi_matrix,
  cell_type = "Inhibitory Neurons"
)

microglia_psi <- subset_sparse_matrix(
  matrix = psi_matrix,
  cell_type = "Microglia"
)

opcs_psi <- subset_sparse_matrix(
  matrix = psi_matrix,
  cell_type = "OPCs"
)

oligodendrocytes_psi <- subset_sparse_matrix(
  matrix = psi_matrix,
  cell_type = "Oligodendrocytes"
)

vascular_psi <- subset_sparse_matrix(
  matrix = psi_matrix,
  cell_type = "Vascular Cells"
)
```

Can also do this for raw SJ counts

```{r split splice junction count matrices by cell type}
# separate sj counts
sj_counts <- setbp1_marvel[["sj.count.matrix"]]

# subset each cell type
astrocytes_sj_count <- subset_sparse_matrix(
  matrix = sj_counts,
  cell_type = "Astrocytes"
)

excitatory_sj_count <- subset_sparse_matrix(
  matrix = sj_counts,
  cell_type = "Excitatory Neurons"
)

inhibitory_sj_count <- subset_sparse_matrix(
  matrix = sj_counts,
  cell_type = "Inhibitory Neurons"
)

microglia_sj_count <- subset_sparse_matrix(
  matrix = sj_counts,
  cell_type = "Microglia"
)

opcs_sj_count <- subset_sparse_matrix(
  matrix = sj_counts,
  cell_type = "OPCs"
)

oligodendrocytes_sj_count <- subset_sparse_matrix(
  matrix = sj_counts,
  cell_type = "Oligodendrocytes"
)

vascular_sj_count <- subset_sparse_matrix(
  matrix = sj_counts,
  cell_type = "Vascular Cells"
)
```

And can do for gene counts

```{r split splice junction count matrices by cell type}
# separate gene counts
gene_counts <- setbp1_marvel[["gene.count.matrix"]]

# subset each cell type
astrocytes_gene_count <- subset_sparse_matrix(
  matrix = gene_counts,
  cell_type = "Astrocytes"
)

excitatory_gene_count <- subset_sparse_matrix(
  matrix = gene_counts,
  cell_type = "Excitatory Neurons"
)

inhibitory_gene_count <- subset_sparse_matrix(
  matrix = gene_counts,
  cell_type = "Inhibitory Neurons"
)

microglia_gene_count <- subset_sparse_matrix(
  matrix = gene_counts,
  cell_type = "Microglia"
)

opcs_gene_count <- subset_sparse_matrix(
  matrix = gene_counts,
  cell_type = "OPCs"
)

oligodendrocytes_gene_count <- subset_sparse_matrix(
  matrix = gene_counts,
  cell_type = "Oligodendrocytes"
)

vascular_gene_count <- subset_sparse_matrix(
  matrix = gene_counts,
  cell_type = "Vascular Cells"
)
```
...
sj_per_cell_type <- data.frame(
  ...
  "sj_number" = c(length(astrocytes_sj_count@Dimnames[[1]]),
                  length(excitatory_sj_count@Dimnames[[1]]),
                  length(inhibitory_sj_count@Dimnames[[1]]),
                  length(microglia_sj_count@Dimnames[[1]]),
                  length(oligodendrocytes_sj_count@Dimnames[[1]]),
                  length(opcs_sj_count@Dimnames[[1]]),
                  length(vascular_sj_count@Dimnames[[1]])),
  
  "sjs_per_cell" = c(length(astrocytes_sj_count@Dimnames[[1]]) /
                              length(astrocytes_sj_count@Dimnames[[2]]),
                  length(excitatory_sj_count@Dimnames[[1]]) /
                              length(excitatory_sj_count@Dimnames[[2]]),
                  length(inhibitory_sj_count@Dimnames[[1]]) /
                              length(inhibitory_sj_count@Dimnames[[2]]),
                  length(microglia_sj_count@Dimnames[[1]]) /
                              length(microglia_sj_count@Dimnames[[2]]),
                  length(oligodendrocytes_sj_count@Dimnames[[1]]) /
                              length(oligodendrocytes_sj_count@Dimnames[[2]]),
                  length(opcs_sj_count@Dimnames[[1]]) /
                              length(opcs_sj_count@Dimnames[[2]]),
                  length(vascular_sj_count@Dimnames[[1]]) /
                              length(vascular_sj_count@Dimnames[[2]])),
  
  "sjs_per_gene" = c(length(astrocytes_sj_count@Dimnames[[1]]) /
                              length(astrocytes_gene_count@Dimnames[[1]]),
                  length(excitatory_sj_count@Dimnames[[1]]) /
                              length(excitatory_gene_count@Dimnames[[1]]),
                  length(inhibitory_sj_count@Dimnames[[1]]) /
                              length(inhibitory_gene_count@Dimnames[[1]]),
                  length(microglia_sj_count@Dimnames[[1]]) /
                              length(microglia_gene_count@Dimnames[[1]]),
                  length(oligodendrocytes_sj_count@Dimnames[[1]]) /
                              length(oligodendrocytes_gene_count@Dimnames[[1]]),
                  length(opcs_sj_count@Dimnames[[1]]) /
                              length(opcs_gene_count@Dimnames[[1]]),
                  length(vascular_sj_count@Dimnames[[1]]) /
                              length(vascular_gene_count@Dimnames[[1]]))
)
```
