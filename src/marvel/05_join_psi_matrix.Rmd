...
# Loop through each file and load it into the environment
for (file in files) {
...
These will be made more pretty in the figure 2 scripts.
...
hist(full_psi_matrix@x)
dev.off()
```

What abut splice junction expression?

We can do counts.

```{r plot nonzero splice junction counts}
...
# save this normalized sj_expression
write_rds(
...
Now that this has been generated, you can plot it.

```{r plot nonzero splice junction expression}
# save and plot nonzero log of sj expression
png(here::here("results", "distributions", "sj_norm_hist.png"),
  width = 8, height = 6, units = "in", res = 300
)
hist(log(normalized_sj_expression@x))
dev.off()
```

Also do the same for gene expression!

```{r plot nonzero values for gene expression}
