...
```{r try rowMeans}
psi_means <- rowMeans(psi_matrix)
hist(psi_means)

sj_means <- rowMeans(setbp1_marvel$sj.count.matrix)
hist(sj_means)

gene_norm_means <- rowMeans(setbp1_marvel$gene.norm.matrix)

gene_count_means <- rowMeans(setbp1_marvel$gene.count.matrix)
```
