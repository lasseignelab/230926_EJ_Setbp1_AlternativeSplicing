...
source(here::here("src", "marvel", "get_psi_df.R"))
...
```{r make son violin plot}
son_data_values <- get_psi_df(
  MarvelObject = setbp1_marvel,
  coord.intron = "chr16:91444873:91448517",
  log2.transform = FALSE
)

son_data_values <- left_join(son_data_values, sample_metadata[, c(1, 6, 17)])

ggplot(son_data_values, aes(x = cell_type, y = psi, fill = seq_folder)) +
  geom_violin()
```
...
##### Run function to fill in psi matrix

The issue with this for loop is that it works, but is too slow to work on all 186,000 splice junctions at once. Therefore, we recommend running this on only a subset of genes/splice junctions.

First, I'll create the subsets. One will have all splice junctions for setbp1, and the other will be all genes with significant psi changes.

```{r create desired subsets}
setbp1_sj <- as.vector(setbp1_marvel[["sj.metadata"]][setbp1_marvel[["sj.metadata"]][["gene_short_name.start"]] == "Setbp1", "coord.intron"])

son_sj <- as.vector(setbp1_marvel[["sj.metadata"]][setbp1_marvel[["sj.metadata"]][["gene_short_name.start"]] == "Son", "coord.intron"])

as_genes <- rownames(gene_lists_mat)
as_genes_sj <- as.vector(setbp1_marvel[["sj.metadata"]][setbp1_marvel[["sj.metadata"]][["gene_short_name.start"]] %in% as_genes, "coord.intron"])
```

```{r use function on subset of splice junctions}
setbp1_psi_matrix <- make_psi_matrix(setbp1_sj)

as_genes_psi_matrix <- make_psi_matrix(as_genes_sj)
```

I also want to run this functions on all splice junctions. Therefore we will create the required list of chromosome names.
...
Now, we need to pivot the data longer so we can plot with ggplot

```{r pivot data}
# convert to dense dataframe
setbp1_psi_df <- as.data.frame(as.matrix(setbp1_psi_matrix))
colnames(setbp1_psi_df) <- sj_counts@Dimnames[[2]]

# pivot longer
setbp1_psi_df <- setbp1_psi_df %>% 
    as_tibble(rownames = "splice_junction") %>%
  pivot_longer(values_to = "psi", cols = colnames(setbp1_psi_df),
               names_to = "cell_id")

# add extra metadata
setbp1_psi_df <- left_join(setbp1_psi_df, sample_metadata, by = c("cell_id" = "cell.id"))

## DO FOR SON< MAKE INTO FUNCTION LATER
son_psi_matrix <- make_psi_matrix(son_sj)
# convert to dense dataframe
son_psi_df <- as.data.frame(as.matrix(son_psi_matrix))
colnames(son_psi_df) <- sj_counts@Dimnames[[2]]

# pivot longer
son_psi_df <- son_psi_df %>% 
    as_tibble(rownames = "splice_junction") %>%
  pivot_longer(values_to = "psi", cols = colnames(son_psi_df),
               names_to = "cell_id")

# add extra metadata
son_psi_df <- left_join(son_psi_df, sample_metadata, by = c("cell_id" = "cell.id"))

```


Make density plots of PSI for setbp1 splice junctions!!

```{r make density plots}
### setbp1
ggplot(setbp1_psi_df, aes(x = log2(psi), color = splice_junction, fill = splice_junction)) +
  geom_density(alpha = 0.2)

ggplot(setbp1_psi_df, aes(x = log2(psi), color = splice_junction, fill = splice_junction)) +
  geom_density(alpha = 0.2) + facet_wrap(seq_folder ~ .)

ggplot(setbp1_psi_df, aes(x = log2(psi), color = splice_junction, fill = splice_junction)) +
  geom_density(alpha = 0.2) + facet_grid(seq_folder ~ cell_type)

ggplot(setbp1_psi_df, aes(x = log2(psi), color = splice_junction, fill = splice_junction)) +
  geom_density(alpha = 0.2) + facet_wrap(cell_type ~ .)

ggplot(setbp1_psi_df, aes(x = log2(psi), color = splice_junction, fill = splice_junction)) +
  geom_density(alpha = 0.2)

### son gene
ggplot(son_psi_df, aes(x = log2(psi), color = splice_junction, fill = splice_junction)) +
  geom_density(alpha = 0.2) + facet_wrap(seq_folder ~ .)
