...
```{r load significant gene tables}
# Load MARVEL object
...
## Learn to extract PSI values

```{r get psi data}
son_psi_table <-
  setbp1_marvel[["adhocGene"]][["Expression"]][["PSI"]][["Table"]]
```

This doesn't have the value per cell..

I looked at the code and there is a violin plot option actually but for the plotvalues.psi function
...
PlotValues.PSI.Pseudobulk.10x(
  ...
  cell.group.list = cell_group_list,
  coord.intron = "chr16:91444873:91448517",
  min.pct.cells.gene.expr = 5
)
```
I keep getting an error here:

Error in data.frame(sample.id = names(cell.group.list.small)[i], n.cells.total = n.cells,  : 
  arguments imply differing number of rows: 0, 1
  
  
I'm going to try to make violinplot from scratch since these functions are not working as planned.
